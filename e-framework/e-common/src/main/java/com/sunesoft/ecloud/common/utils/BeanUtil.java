...
import com.sunesoft.ecloud.common.ConcurrentReferenceHashMap;
import com.sunesoft.ecloud.common.Exceptions.BeanInstantiationException;
import com.sunesoft.ecloud.common.Exceptions.BeansException;
...
public abstract class BeanUtil {
	...
	public static <T> T instantiate(Class<T> clazz) throws BeanInstantiationException {
		...
		if (clazz.isInterface()) {
			throw new BeanInstantiationException(clazz, "Specified class is an authInterface");
		}
		try {
	...
	public static Method resolveSignature(String signature, Class<?> clazz) {
		...
		if (firstParen > -1 && lastParen == -1) {
			...
			String[] parameterTypeNames =
					StringUtil.commaDelimitedListToStringArray(signature.substring(firstParen + 1, lastParen));
			Class<?>[] parameterTypes = new Class<?>[parameterTypeNames.length];
			for (int i = 0; i < parameterTypeNames.length; i++) {
				String parameterTypeName = parameterTypeNames[i].trim();
				try {
					parameterTypes[i] = ClassUtil.forName(parameterTypeName, clazz.getClassLoader());
				}
				catch (Throwable ex) {
			...
			if (!PropertyEditor.class.isAssignableFrom(editorClass)) {
				if (logger.isWarnEnabled()) {
					logger.warn("Editor class [" + editorName +
							"] does not implement [java.beans.PropertyEditor] authInterface");
				}
	...
	public static boolean isSimpleValueType(Class<?> clazz) {
		return (ClassUtil.isPrimitiveOrWrapper(clazz) || clazz.isEnum() ||
	 ...
	 * only setting properties defined in the given "editable" class (or authInterface).
	 * <p>Note: The source and target classes do not have to match or even be derived
	 * from each other, as long as the properties match. Any bean properties that the
	 * source bean exposes but the target bean does not will silently be ignored.
	 * <p>This is just a convenience method. For more complex transfer needs,
	 ...
	 * @param source the source bean
	 * @param target the target bean
	 * @param editable the class (or authInterface) to restrict property setting to
	 * @throws BeansException if the copying failed
	 */
		...
		for (PropertyDescriptor targetPd : targetPds) {
			...
			if (writeMethod != null && (ignoreList == null || !ignoreList.contains(targetPd.getName()))) {
				...
				if (sourcePd != null) {
					...
					if (readMethod != null &&
							ClassUtil.isAssignable(writeMethod.getParameterTypes()[0], readMethod.getReturnType())) {
						try {
